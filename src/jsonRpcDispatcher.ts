...
export class JsonRpcDispatcher {
    ...
    private readonly pendingRequests = new Map<JsonRpcMessageId, ResultLatch<JsonRpcMethodResponse>>();
    private readonly exposedMethods = new Map<string, JsonRpcExposedMethod>();
    private readonly notificationListeners = new Map<string, JsonRpcNotificationListener>();
    private messageIdCounter: number = 0;
    private transport: JsonRpcTransportAdapter;
    private readonly messageIdFactory: () => JsonRpcMessageId = () => this.defaultMessageIdFactory();
    ...
    async connect(transport: JsonRpcTransportAdapter) {
        ...
        this.transportSubscriptions
            .add(
                this.transport.addListener((message) => this.dispatchMessage(message))
            )
            .add(
    ...
    exposeMethodWithArgs(method: string, args: number | boolean, handler: JsonRpcMethodHandler) {
        this.exposedMethods.set(method, {method, params: {positionalParams: args}, handler});
    }
    ...
    async request(method: string, params?: JsonRpcMessageParams): Promise<JsonRpcMethodResponse> {
    ...
    async invoke(method: string, params?: JsonRpcMessageParams): Promise<any> {
        ...
        if (response.error) {
            throw new JsonRpcInvocationException('Failed to invoke RPC method: ' + method, response.error);
        }
    ...
    private dispatchMessage(jsonMessage: string) {
        ...
        try {
            ...
            this.logger.captureError(error, 'Failed to parse JSON RPC message', jsonMessage);
    ...
    private async onNotification(notification: JsonRpcNotification) {
        ...
        const validationErrorResponse = this.validateRequestParams(notification.params, listener.params);
        ...
        try {
            ...
            this.logger.captureError(error, 'Error during invocation of a notification listener');
        }
    }
    ...
    private async onRequest(request: JsonRpcRequest) {
        ...
        const requestResponse: JsonRpcResponse = {jsonrpc: JSON_RPC_VERSION, id: request.id};

        const validationErrorResponse = this.validateRequestParams(request.params, exposedMethod.params);
        if (validationErrorResponse) {
        ...
        try {
            ...
            const directResult: any = await exposedMethod.handler(request.params, methodResponse);
    ...
    private terminateAllPendingRequests() {
        ...
        this.pendingRequests.forEach(latch => {
            try {
                ...
                this.logger.captureError(error, 'Failed to terminate a pending request');
            }
        });
    ...
    private validateRequestParams(params: JsonRpcMessageParams, expected: JsonRpcExpectedParams | boolean | null): JsonRpcError | null {
        if (!expected) {
            return null;
        }
        ...
        if (expected === true) {
        ...
        } else if (expected.positionalParams === true ||
            expected.positionalParams === 0 || expected.positionalParams > 0) {
            if (!isArray) {
                return expected.positionalParams === 0 ? null : RpcErrors.InvalidParamsError;
            }
            const positionalParams = params as any[];
            if (expected.positionalParams !== true && expected.positionalParams > positionalParams.length) {
            ...
            if (Values.isArray(expected.namedParams)) {
                ...
                const invalid = expected.namedParams.some(key => namedParams[key] === undefined);
                if (invalid) {
                    return RpcErrors.InvalidParamsError;
                }
            }
...
namespace RpcErrors {
    ...
    export function createMethodNotFoundResponse(requestId: JsonRpcMessageId): Readonly<JsonRpcResponse> {
        return {
            jsonrpc: JSON_RPC_VERSION,
            id: requestId,
            error: {
